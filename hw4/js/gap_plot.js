...
class GapPlot {
    ...
    updatePlot(activeYear, xIndicator, yIndicator, circleSizeIndicator) {
        ...
        /*
        You will be updating the scatterplot from the data. 
        hint: use the #chart-view div

        *** Structuring your PlotData objects ***
        You need to start by mapping the data specified by the parameters to 
        the PlotData Object. Your PlotData object is specified at the top of 
        the file. You will need get the data specified by the x, y and circle size 
        parameters from the data passed to the GapPlot constructor
        */
        let plotData = [];
        for(let c of this.data.population){
            let country = c.country;
            let id = c.geo.toUpperCase();
            let region = c.region;
            let xVals = this.data[xIndicator];
            let xVal = "tbd"
            for(let x of xVals){
            ...
            let yVals = this.data[yIndicator];
            let yVal;
            for(let y of yVals){
            ...
            let circleVals = this.data[circleSizeIndicator];
            let circleSize;
            for(let c of circleVals){
                if(c.country === country){
                    circleSize = c[activeYear];
                }
            }
            let pd = new PlotData(country,xVal,yVal,id,region,circleSize)
            plotData.push(pd);
        }
        ...
        *** Setting the scales for your x, y, and circle data ***
        For x and y data, you should get the overall max of the whole data set 
        for that data category, not just for the activeYear.
        
        let xValues = this.data[xIndicator].map(country => {
            console.log(country);
            let values = [];
            for(let year of country){
                if(parseInt(year,10) !== NaN){
                    values.push(country[year])
                }
            }
            return values;
        });
        ...
        /*
        ***draw circles***
        draw the circles with a scaled area from the circle data, with cx from 
        your x data and cy from y data. You need to size the circles from your 
        circleSize data, we have provided a function for you to do this
        called circleSizer. Use this when you assign the 'r' attribute.

        ***Tooltip for the bubbles***
        You need to assign a tooltip to appear on mouse-over of a country 
        bubble to show the name of the country. We have provided the 
        mouse-over for you, but you have to set it up.
